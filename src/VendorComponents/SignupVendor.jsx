...
import { Store } from "lucide-react"; // For the store icon
...
export default function SignupVendor() {
                ...
                try {
                    ...
                    setForm((prev) => ({
                        ...
                        address: {
                            ...
                            country: address.country || "", // Let's not default to "India" here, allow it to be empty if not found
                        },
                    }));
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.name || !form.email || !form.password || !form.phone || !form.shopName || !form.businessType ||
            !form.address.pincode || !form.address.state || !form.address.district || !form.address.country) {
            setSignupError("Please fill in all required fields.");
            return;
        }
        if (!form.shopImage) {
            setSignupError("Please upload a shop image.");
            return;
        }
        ...
        try {
            ...
            if (registerVendor.fulfilled.match(result)) {
                showModal("Vendor registration successful! Redirecting to dashboard.");
                navigate("/vendorDashboard");
            } else if (registerVendor.rejected.match(result)) {
                const errorMessage = result.payload?.message || "Vendor registration failed. Please try again.";
    ...
    return (
        ...
        >
            ...
            <div className="w-full max-w-xl p-8 flex-grow-0" // Removed rounded-2xl, shadow-xl, bg-white, border classes
                style={{
            ...
            >
                ...
                <div className="text-center mb-8">
                    ...
                    {/* Vendor specific title and subtitle */}
                    <h2 className="text-3xl font-extrabold mb-2 tracking-tight mt-6" style={{ fontFamily: "'Playfair Display', serif", color: textColor }}>Become a Vendor</h2> {/* Adjusted font size */}
                    <p className="text-lg" style={{ fontFamily: "'Playfair Display', serif", color: textColor }}>Register your shop and start selling with us!</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6"> {/* Increased spacing */}
                    ...
                    <div className="space-y-4 pt-4 border-t" style={{ borderColor: cardBorderColor }}>
                        <h3 className="text-xl font-semibold flex items-center" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>
                            <Store className="mr-2" style={{ color: primaryGreenLight }} /> Shop Details
                        </h3>
                        <div>
                            <label htmlFor="shopName" className="block text-sm font-medium mb-1" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>Shop Name</label>
                            <input
                                name="shopName"
                                id="shopName"
                        ...
                        <div>
                            <label htmlFor="businessType" className="block text-sm font-medium mb-1" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>Business Type</label>
                            <input
                                name="businessType"
                                id="businessType"
                                placeholder="e.g., Groceries, Electronics, Restaurant"
                        ...
                        <div>
                            <label htmlFor="shopImage" className="block text-sm font-medium mb-1" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>Shop Image (Upload)</label>
                            <input
                                name="shopImage"
                                id="shopImage"
                                type="file"
                    ...
                    <p className="text-sm text-center mt-6" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>
                        Already registered?{" "}
                        <Link to="/vendorlogin" className="font-semibold hover:underline" style={{ color: primaryGreenLight, fontFamily: "'Playfair Display', serif" }}>
                            Login here
                        </Link>
                    </p>
                </form>
