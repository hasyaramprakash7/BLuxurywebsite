...
import { registerDeliveryBoy } from "../features/delivery/deliveryBoySlice";
import { useNavigate, Link } from "react-router-dom";
import axios from "axios";
import { Bike } from "lucide-react"; // Using Bike icon for delivery boy
import { FaEye, FaEyeSlash, FaMapMarkerAlt, FaSpinner } from "react-icons/fa"; // For other icons (install react-icons)
...
export default function SignupDeliveryBoy() {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { loading, error } = useSelector((state) => state.deliveryBoyAuth);
    ...
    const [form, setForm] = useState({
        ...
        password: "",
        phone: "",
        profileImage: null, // Renamed for delivery boy context
        address: {
            latitude: "",
            longitude: "",
            pincode: "",
    ...
    const [loadingAddress, setLoadingAddress] = useState(false); // State for address loading
    ...
    useEffect(() => {
        ...
        setLoadingAddress(true); // Start loading indicator for address
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                try {
                    ...
                    setLoadingAddress(false); // End loading indicator
                }
            },
            (err) => {
            ...
            if (res.data.length > 0) {
                ...
                setForm((prev) => ({
                    ...
                    address: {
                        ...
                        latitude: location.lat || prev.address.latitude,
                        longitude: location.lon || prev.address.longitude,
                    },
                }));
    ...
    const handleChange = (e) => {
        ...
        if (name === "profileImage") { // Changed from shopImage
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.profileImage) { // Changed from shopImage
            setSignupError("Please upload a profile image.");
            return;
        }
        ...
        // Removed shopName and businessType as they are not relevant for delivery boys
        formData.append("profileImage", form.profileImage); // Changed from shopImage
        formData.append(
            "address",
            JSON.stringify({
                ...form.address,
        ...
        try {
            ...
            const result = await dispatch(registerDeliveryBoy(formData));
            if (registerDeliveryBoy.fulfilled.match(result)) {
                showModal("Delivery boy registration successful! Redirecting to dashboard.");
                navigate("/delivery-dashboard");
            } else if (registerDeliveryBoy.rejected.match(result)) {
                const errorMessage = result.payload?.message || "Delivery boy registration failed. Please try again.";
                setSignupError(errorMessage);
            }
        } catch (err) {
            console.error("Delivery boy registration failed:", err);
    ...
    return (
        ...
        >
            ...
            <div className="w-full max-w-xl p-8 flex-grow-0" // Removed rounded-2xl, shadow-2xl, bg-white, border classes
                style={{
            ...
            >
                ...
                <div className="text-center mb-8">
                    ...
                    <h2 className="text-3xl font-extrabold mb-2 tracking-tight mt-6" style={{ fontFamily: "'Playfair Display', serif", color: textColor }}>Become a Delivery Partner</h2> {/* Adjusted font size */}
                    <p className="text-lg" style={{ fontFamily: "'Playfair Display', serif", color: textColor }}>Join our team and start delivering!</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6"> {/* Increased spacing */}
                    ...
                    <div className="space-y-4 pt-4 border-t" style={{ borderColor: cardBorderColor }}>
                        <h3 className="text-xl font-semibold flex items-center" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>
                            <Bike className="mr-2" style={{ color: primaryGreenLight }} /> Profile Image
                        </h3>
                        <div>
                            <label htmlFor="profileImage" className="block text-sm font-medium mb-1" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>Upload your Profile Picture</label>
                            <input
                                name="profileImage"
                                id="profileImage"
                                type="file"
                                ...
                                onChange={handleChange}
                                required
                                className="w-full text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 transition duration-200" // Adjusted file input styling for light theme
                    ...
                    <button
                        type="submit"
                        disabled={loading || loadingAddress} // Disable if Redux loading or address loading
                    ...
                    {error && <p className="text-sm font-medium mt-4 p-2 rounded-lg border text-center"
                        style={{
                            color: errorColor, // Red error text
                            backgroundColor: "rgba(239, 68, 68, 0.1)", // Light red transparent background
                            borderColor: errorColor,
                    ...
                    <p className="text-sm text-center mt-6" style={{ color: textColor, fontFamily: "'Playfair Display', serif" }}>
                        ...
                        <Link to="/delivery-login" className="font-semibold hover:underline" style={{ color: primaryGreenLight, fontFamily: "'Playfair Display', serif" }}>
                            Login here
                        </Link>
                    </p>
                </form>
